...
def convert_residue_to_AAIndex(residue):
    ...
    letterDict["Z"] = [0.149, -0.184, -0.030, 0.035, -0.112]
...
def get_all_residue_PSSM(complex_name, PSSM_path):
    ...
    """
    PSSM_r_path = os.path.join(PSSM_path, complex_name + "_r_b", "pssm")
    PSSM_l_path = os.path.join(PSSM_path, complex_name + "_l_b", "pssm")
    ...
    for chain_name in r_chain_name:
        path = os.path.join(PSSM_r_path, complex_name + "_r_b." + chain_name + ".pdb.pssm")
    ...
    for chain_name in l_chain_name:
        path = os.path.join(PSSM_l_path, complex_name + "_l_b." + chain_name + ".pdb.pssm")
        with open(path, 'r') as f:
            line = f.readline()
            while line:
                if line.split()[0] != 'pdbresi':
...
if __name__ == '__main__':
    ...
    pdb_path = os.path.join(fir_dir, "1_bm4_dataset", "2_modify_pdb_result")
    PSSM_path = os.path.join(fir_dir, "2.1_native_PSSM")
    node_feature_path = os.path.join(fir_dir, '7_node_feature', 'native_node_feature')
    target_name = os.path.join(fir_dir, 'caseID.lst')
    ...
    for target in all_target_name:
        ...
        ###1.find nodeid of native receptor
        r_decoy_path = os.path.join(pdb_path, target + '_r_b.pdb')
        r_atomic_data = form_dataframe(r_decoy_path)
        r_dataframe = pd.DataFrame(r_atomic_data)

        r_nodeid_initial_list = r_dataframe["node_id"].tolist()
        r_nodeid_list = list(set(r_nodeid_initial_list))
        r_nodeid_list.sort(key=r_nodeid_initial_list.index)

        l_decoy_path = os.path.join(pdb_path, target + '_l_b.pdb')
        l_atomic_data = form_dataframe(l_decoy_path)
        l_dataframe = pd.DataFrame(l_atomic_data)

        l_nodeid_initial_list = l_dataframe["node_id"].tolist()
        l_nodeid_list = list(set(l_nodeid_initial_list))
        l_nodeid_list.sort(key=l_nodeid_initial_list.index)

        nodeid_list = r_nodeid_list + l_nodeid_list
