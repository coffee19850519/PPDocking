...
class EarlyStopping:
    ...
    def __call__(self,
        ...
        if mode == 'max':
            if self.best_score is None:
                ...
                self.save_checkpoint_process(val_metric,path, model, scaler, args)
    ...
    def save_checkpoint_process(self,val_loss, path, model, scaler, args):
...
def compute_PCC(valid_preds, valid_targets, complex_names, mean, std):
    ...
    for i in range(len(all_complex_names)):
        ...
        if len(current_complex_results) > 1:
            corr, _ = pearsonr(current_complex_results['pred'] * std + mean,
                               current_complex_results['target']* std + mean)
        else:
            continue
...
def compute_ratio_EF(valid_preds, valid_targets, complex_names, ratio_list, save_folder):
    ...
    for complex_name in all_complex_names:
        ...
        for EF_percent in ratio_list:
            k = math.ceil(EF_percent * df.shape[0])
            top_dataframe = df.iloc[0:k]
            top_accuracy = (top_dataframe["target"].sum(axis=0)) / (top_dataframe.shape[0])
            top_EF = top_accuracy * EF_max
            top_EF_dic[EF_percent] = top_EF

        top_list.append(top_EF_dic)
        del top_EF_dic

    top_dataframe = pd.DataFrame(top_list, columns=['complex_names'] + ['EF_max'] + ratio_list)
    ...
    EF_percent_dic['EF_max'] = np.mean(top_dataframe['EF_max'])
    for EF_percent in ratio_list:
        EF_percent_dic[EF_percent] = np.mean(top_dataframe[EF_percent])
    ...
    #path = os.path.join(fir_dir, 'enrichment_result', '20200330-2', 'enrichment_factor-1.csv')
    #makedirs(path, isfile=True)
    df_all.to_csv(os.path.join(save_folder, 'ratio_EF.csv'), index=False)

    for key, values in EF_percent_dic.items():
        print(str(key) + ':' + str(values))

def compute_EF1(valid_preds, valid_targets, complex_names):
    ...
    for complex_name in all_complex_names:
        ...
        if df["target"].sum(axis=0) != 0:
            EF_max = (df.shape[0]) / (df["target"].sum(axis=0))
            k = math.ceil(0.006 * df.shape[0])
            top_dataframe = df.iloc[0:k]
            top_accuracy = (top_dataframe["target"].sum(axis=0)) / (top_dataframe.shape[0])
            top_EF = top_accuracy * EF_max
            ef_list.append(top_EF)
        else:
            #print('{:s} has no positive label'.format(complex_name))
            continue
    ef1 = np.mean(ef_list)
    del dict, source_data, all_complex_names,ef_list
    return ef1