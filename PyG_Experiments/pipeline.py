...
from PyG_Experiments.compute_enrichment_factor import calculate_average_EF_from_meta_learners, calculate_successful_rate, calculate_average_hit_count
from PyG_Experiments.training_and_testing import train_classification,train_regression,test_classification,test_regression,val_classification
...
class arguments:
    def __init__(self, num_node_features, num_edge_attr,
                 ...
                 batch_size, fold_num, pos_weights,learning_rate, device,savecheckpoints_mode,savecheckpoints_monitor, earlystop_patience,epochs,
                 train_classification_patch_path,val_classification_patch_path,test_classification_patch_path,
                 train_subset_number,test_max_subset_number,val_max_subset_number,
                 root
                 ):
...
if __name__ == '__main__':
    args = arguments(
                ...
                savecheckpoints_monitor = "val_rpc+successrate",
    ...
    model_save_name = "200726_trainval_3pos_globalpool_CGCN_4edgefeature_3layers_valtest_alldata_vrpc_savecheckpoint+1trainsubset"
    model_save_folder = os.path.join(checkpoints_folder, model_save_name)
    ...
    for n in range(0, args.fold_num):
        ...
        val_temp = glob.glob(os.path.join(args.val_classification_patch_path,
                        '{:d}_subset_{:d}_fold_*_patch_val_data.pt'.format(args.val_max_subset_number, n)))
        test_temp = glob.glob(os.path.join(args.test_classification_patch_path,
                        '{:d}_subset_{:d}_fold_*_patch_test_data.pt'.format(args.test_max_subset_number, n)))
        train_temp = glob.glob(os.path.join(args.train_classification_patch_path,
                        '{:d}_subset_{:d}_fold_*_patch_train_data.pt'.format(args.train_subset_number, n)))

        train_patch_size = len(train_temp)
        val_patch_size = len(val_temp)
        test_patch_size = len(test_temp)

        del train_temp, val_temp, test_temp

        if args.mode == 'classification':
            training_model, summary = train_classification(args = args, model=training_model, fold=n, model_save_folder=model_save_folder, train_patch_size=train_patch_size,val_patch_size=val_patch_size)
        ...
        training_model, _, _ = load_checkpoint(os.path.join(model_save_folder, '{:d}_fold_{:s}_model.pt'.format(n, args.mode)), training_model)
        # split = "test"
        if args.mode == 'classification':
            ...
            test_loss, test_auc, test_rpc, test_ef, complex_names, decoy_names, scores, targets = test_classification(
                args=args, model = training_model, fold=n, split = "test", max_subset_number = args.test_max_subset_number,patch_size = test_patch_size)
            ...
            del test_loss, test_auc, test_rpc, test_ef1, complex_names, decoy_names, scores, targets
        elif args.mode == 'regression':
            test_loss, test_mae, test_pcc, test_ef1, _, _, _, _ = test_regression(load_model, args.device, test_loader, mean, std)
