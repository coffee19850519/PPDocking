...
def read_decoy_labels_and_correspondence(label_folder, target_name):
    ...
    with open(os.path.join(os.path.splitext(decoy_name_path)[0] + '_positive_correspondence.json'), 'r') as fp:
        pos_correspondence_gt = json.load(fp)
    with open(os.path.join(os.path.splitext(decoy_name_path)[0] + '_negative_correspondence.json'), 'r') as fp:
        neg_correspondence_gt = json.load(fp)

    return decoy_name_df, pos_correspondence_gt, neg_correspondence_gt
...
def convert_decoys_in_target_to_pt(target,label_folder, pdb_folder,node_feature_folder, pre_transform, interface_folder = None):
    ...
    print(target + ": start!")
    # read decoy labels
    target_lables, pos_correspondence_gt, neg_correspondence_gt = read_decoy_labels_and_correspondence(label_folder, target)
    data_list = []
    ...
    for decoy_pdb in target_lables.No.tolist():
        if len(pos_correspondence_gt[decoy_pdb]) == 0 or len(neg_correspondence_gt[decoy_pdb]) == 0:
            continue
        try:
            ...
            for current_correspondence in  pos_correspondence_gt[decoy_pdb]:
            ...
            for current_correspondence in neg_correspondence_gt[decoy_pdb]:
                full_name_correspondence.append([
                    target + '.' + os.path.splitext(decoy_pdb)[0] + '.' + current_correspondence[0],
                    target + '.' + os.path.splitext(decoy_pdb)[0] + '.' + current_correspondence[1]
                ])
            ...
            data = Data(x=torch.from_numpy(np.array(atom_features, dtype=np.float)),
                        # y=torch.from_numpy(np.array(category, dtype=np.uint8)).view(1, -1),
                        ...
                        coupling_categories = torch.from_numpy(np.array(category, dtype=np.uint8)),
                        # fnat=torch.tensor(decoy_labels['fnat'], dtype=torch.float).view(1, -1),
            ...
            del atom_features, data, decoy_labels, edge_index, edge_attr, residue_num_list, atom_coordinates, full_name_correspondence, chain_list, residue_features, category
        except Exception as e:
            ...
            continue
            # return []
    del target_lables, pos_correspondence_gt, neg_correspondence_gt
    print(target + ": end!")
    return data_list
