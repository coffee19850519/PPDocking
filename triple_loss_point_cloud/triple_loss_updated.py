...
class DockPointNet(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        # t_eye_A = torch.eye(t_matrix_A.shape[-1]).expand_as(t_matrix_A).to(t_matrix_A.device)
        # t_matrix_A = torch.matmul(t_matrix_A,torch.transpose(t_matrix_A,1,2))
        # t_dif_A = t_eye_A-t_matrix_A + 1e-8
        # t_loss_A = torch.norm(t_dif_A,2,(1,2))
        # t_loss_A = torch.sum(t_loss_A) + 1e-8
        #
        # t_eye_B = torch.eye(t_matrix_B.shape[-1]).expand_as(t_matrix_B).to(t_matrix_B.device)
        # t_matrix_B = torch.matmul(t_matrix_B, torch.transpose(t_matrix_B, 1, 2))
        # t_dif_B = t_eye_B - t_matrix_B
        # t_loss_B = torch.norm(t_dif_B, 2, (1, 2))
        # t_loss_B = torch.sum(t_loss_B)
