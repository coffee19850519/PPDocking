...
from cls_post_pretrain.single_input_convertor import PPDocking_point_cloud
...
def train_classification(args, model, fold, model_save_folder, train_patch_size, train_subset_number, val_subset_number,earlystop,learning_rate, weight_decay):#,  mean, std):
    ...
    for epoch in range(1, args.epochs):
        ...
        for step, data in enumerate(tqdm(train_loader)):
            ...
            total_loss += float(loss) * len(data.complex_name)
            sample_length += len(data.complex_name)
            optimizer.step()
        ...
        val_loss = val_classification(args, model, val_loader)
            # , all_complex_names, all_decoy_names, all_scores, all_targets, auc, auc_precision_recall,ef,sr
        ...
        print('Epoch: {:03d}, LR: {:4f}, Loss: {:.4f},  val_loss: {:.4f}'.format(epoch,lr,average_loss,val_loss))
        summary.append(
            {'fold': fold, 'epoch': epoch, 'lr': lr, 'train_loss': loss.item(), 'val_loss': val_loss})
...
def val_classification(args, model, test_loader):
    ...
    for data in tqdm(test_loader):
        ...
        with torch.no_grad():
            ...
            all_val_loss += float(val_loss) * len(data.complex_name)
        # all_scores.append(out)
        # all_targets.append(y)
        coupling_num += len(data.complex_name)
