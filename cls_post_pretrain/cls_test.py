...
from torch.nn import Sequential as Seq, Dropout, Conv2d, Conv1d, Linear as Lin, ReLU, LeakyReLU, BatchNorm1d as BN, \
    LayerNorm, Embedding, CosineEmbeddingLoss, TripletMarginLoss,BCEWithLogitsLoss
...
def global_index_residue(residue_list_batch_A, decoy_list, device):
    ...
    for decoy_idx, decoy in enumerate(decoy_list):
        ...
        residue_in_decoy.extend(np.repeat(decoy_idx, len(set(residue_list_batch_A[decoy_idx]))).tolist())
    ...
    return torch.tensor(residue_index_batch_decoy, dtype=torch.long, device=device), \
...
def global_index_residue2(residue_list_batch, decoy_list, device):
...
def organize_by_correspondence(x, residue_list, decoy_list, correspondence):
    ...
    for sample_idx in range(len(decoy_list)):
        ...
        current_native_residues = residue_list[sample_idx]
        native_residue_set = list(dict.fromkeys(current_native_residues))


        for native_A_edge, native_B_edge, category in current_correspondence:
            categories.append(category)
            native_A_index = native_residue_set.index(native_A_edge) + index_pad_complex
            native_B_index = native_residue_set.index(native_B_edge) + index_pad_complex

            complex_indices_A.append(native_A_index)
            complex_indices_B.append(native_B_index)

        index_pad_complex += len(native_residue_set)

        del native_residue_set

    residue_A_embeddings = x.index_select(dim=0,
                                           index=torch.tensor(complex_indices_A, dtype=torch.long, device=x.device))
    residue_B_embeddings = x.index_select(dim=0,
                                           index=torch.tensor(complex_indices_B, dtype=torch.long, device=x.device))


    coupling_edges = torch.cat((residue_A_embeddings, residue_B_embeddings), dim=1)

    categories = torch.tensor(categories, dtype=torch.float, device=x.device).view(-1, 1)
    return coupling_edges, categories
...
class DockPointNet(torch.nn.Module):
    def __init__(self, in_channels, edge_dim, out_channels):
        ...
        # may want to set dropout to lower value here
        self.fcc = Lin(1024,1)
        ...
        # self.loss = BinaryFocalLoss(alpha=4)
        self.loss = BCEWithLogitsLoss(pos_weight= torch.tensor(4, dtype= torch.float))
        self.reset_parameters()
        # self.mlp = Seq(
    ...
    def forward(self, data):
        ...
        x, pos, normal, batch, residue_list, edge_index, edge_attr, correspondence, decoy_list = data.x.float(), data.pos.float(), data.norm.float(), data.batch, data.residue_index_list, \
                                                                                     data.edge_index, data.edge_attr, data.correspondence, data.decoy_name

        x_A2 = self.conv2(x, pos, batch, None, normal)
        x_A3 = self.conv3(x, pos, batch, None, normal)
        x_A4 = self.conv4(x, pos, batch, None, normal)
        ...
        res_x_A = global_max_pool(atom_x_A, residue_index_batch)

        res_x_A = self.res_embed_layers(res_x_A)

        residue_edges, categories = organize_by_correspondence(res_x_A,residue_list,decoy_list,correspondence)

        out = self.fcc(residue_edges)
        # out = torch.sigmoid(out)

        loss = self.loss(out, categories)
