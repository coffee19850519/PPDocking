...
from pairwise_point_cloud.pair_point_cloud_convertor import PPDocking_point_cloud
...
def print_l2_loss(model):
    l2_loss = 0
    for param in model.parameters():
        l2_loss += 0.5 * torch.sum(param ** 2)
    print('regularization loss is {:.4f}'.format(l2_loss))

def train_classification(args, model, fold, model_save_folder, train_patch_size, train_subset_number, val_subset_number,earlystop,learning_rate, weight_decay):#,  mean, std):
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, follow_batch=['x_A', 'x_B'], shuffle=False, num_workers=0)
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, follow_batch=['x_A', 'x_B'], shuffle=True, num_workers=0)
    ...
    for epoch in range(1, args.epochs):
        ...
        for step, data in enumerate(tqdm(train_loader)):
            ...
            train_summary.append({'fold': fold, 'epoch': epoch, 'step': step, 'train_loss': loss.item()})

            del data

        average_loss = total_loss / (sample_length + 0.00001)
        #, all_scores, all_targets, val_ef, val_success_rate
        val_loss, all_complex_names, all_decoy_names, all_scores, all_targets, auc, auc_precision_recall,ef,sr = val_classification(args, model, val_loader)
        ...
        print('Epoch: {:03d}, LR: {:4f}, Loss: {:.4f},  val_loss: {:.4f}, val_auc:{:.4f}, val_prc:{:.4f}, val_ef:{:.4f}, val_sr:{:.4f}'.format(epoch,
                                                                                                                 lr,
                                                                                                                 average_loss,
                                                                                                                 val_loss,
                                                                                                                 auc,
                                                                                                                 auc_precision_recall, ef, sr))
        summary.append(
            {'fold': fold, 'epoch': epoch, 'lr': lr, 'train_loss': loss.item(), 'val_loss': val_loss, 'val_auc': auc,
             'val_prc': auc_precision_recall, 'val_ef':ef, 'val_sr':sr})

        del  val_loss, all_complex_names, all_scores, all_targets, ef, sr
    ...
    with open(os.path.join(model_save_folder, r'train_summary.json'), mode= 'a') as fp:
        json.dump(train_summary, fp)
...
def val_classification(args, model, test_loader):
    ...
    for data in tqdm(test_loader):
        ...
        all_quality_marker.extend(np.array(data.y_A).reshape(-1).tolist())
        data = data.to(args.device)
        with torch.no_grad():
            # out, node_significance, node_belong_decoy_num, interface_pos, decoy_name = model(data, data.num_graphs)
            val_loss, out, y, fnat = model(data)
            # print('current val_loss is {:.4f}'.format(val_loss))
    ...
    # fpr, tpr, _ = metrics.roc_auc_score(all_targets, all_scores, pos_label=1)
    auc = metrics.roc_auc_score(all_targets, all_scores,average='weighted')
    # precision, recall, _ = metrics.precision_recall_curve(all_targets, all_scores, pos_label=1)
    auc_precision_recall = metrics.average_precision_score(all_targets, all_scores,average='weighted', pos_label= 1)
    val_loss = all_val_loss / (coupling_num + 0.00001)
    ef = compute_EF(predict_fnat, all_quality_marker, all_complex_names, 20)
    success_rate = compute_success_rate(predict_fnat, all_quality_marker, all_complex_names,20)
    del all_val_loss, all_quality_marker
    ...
    return val_loss, all_complex_names, all_decoy_names, all_scores, all_targets, auc, auc_precision_recall, ef, success_rate
...
def test_classification(args, model, fold, split,max_subset_number,patch_size):
    ...
    for test_patch_number in range(patch_size):
        ...
        for data in tqdm(test_loader):
            ...
            with torch.no_grad():
                ...
                #
                # del node_significance, node_belong_decoy_num, interface_pos, decoy_name
                test_loss, out, y, fnat = model(data)
                # out = torch.sigmoid(out)
                #val_loss = sigmoid_focal_loss(out, data.y, reduction='mean')
    ...
    return test_loss, all_complex_names, all_decoy_names, all_scores, all_targets, auc, auc_precision_recall, ef, success_rate, all_quality_marker, predict_fnat
...
class arguments:
    def __init__(self, num_node_features, num_edge_attr,regression_label_type,
                 ...
                 batch_size, fold_num, pos_weights, learning_rate,weight_decay, device, savecheckpoints_mode,
