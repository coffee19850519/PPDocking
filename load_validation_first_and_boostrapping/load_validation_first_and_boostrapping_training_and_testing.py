...
def train_classification(args, model, fold, model_save_folder, train_patch_size, train_subset_number, val_subset_number,earlystop,learning_rate, weight_decay):#,  mean, std):
    ...
    for epoch in range(1, args.epochs):
        ...
        val_loss, all_complex_names, all_decoy_names, all_scores, all_targets, auc, auc_precision_recall, test_ef, test_sr = val_classification(args, model, val_loader)
        ...
        print('Epoch: {:03d}, LR: {:4f}, Loss: {:.4f},  val_loss: {:.4f}, val_auc:{:.4f}, val_prc:{:.4f}, test_ef: {:.4f}, test_sr: {:.4f}'.format(epoch, lr, average_loss, val_loss, auc,auc_precision_recall, test_ef, test_sr))
        summary.append({'fold':fold, 'epoch':epoch, 'lr':lr, 'train_loss':loss.item(), 'val_loss': val_loss, 'val_auc': auc, 'val_prc':auc_precision_recall, 'ef': test_ef, 'sr': test_sr})
...
def val_classification(args, model, test_loader):
    ...
    for data in tqdm(test_loader):
        ...
        all_quality_marker.extend(np.array(data.y).reshape(-1).tolist())
        data = data.to(args.device)
        with torch.no_grad():
    ...
    success_rate,_ = compute_success_rate(predict_fnat, all_quality_marker, all_complex_names, 20)
    del all_val_loss
...
def test_classification(args, model, fold, split,max_subset_number,patch_size):
    ...
    # del all_test_loss
    # return val_loss, auc, auc_precision_recall, all_complex_names, all_decoy_names, all_scores, all_targets, test_acc#
