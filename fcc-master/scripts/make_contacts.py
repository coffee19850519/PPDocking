...
def _calculate_contacts(executable, pdbfile, d_cutoff, filter_selection=None, extension='.contacts'):
    ...
    contacts = sorted(list(set([l for l in p_output.decode().split('\n')][:-1])))
...
if __name__ == '__main__':
    ...
    USAGE = "%s [-f structures.txt] [-n 4] [-c 5.0] [-p decoy_path] file1.pdb file2.pdb"
    ...
    parser.add_option('-p', '--path', dest="decoy_path", action='store', type='string',
                      default=None,
                      help='The path of decoy samples')
    ...
    args = list(map(lambda x:options.decoy_path +'/'+x, args))
