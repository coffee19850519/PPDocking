...
# from proteingraph import ProteinGraph,InterfaceGraph
import json
...
def get_node_feature(folder, node_list, target_name, decoy_number):
    ...
    with open(os.path.join(folder, target_name + '.json')) as json_file:
        node_complex_feature_dic = json.load(json_file)          ###PSSM+AAidex
...
def process_classification(pdb_folder, GLY_side_chain_folder, interface_count_folder,  node_feature_folder,
    ...
    data = Data(edge_index=edge_index,
                ...
                y=torch.tensor(float(decoy_label), dtype=torch.float).view(1, -1),
    ...
    torch.save(data, r'/home/fei/Research/processed_all_data/classification/{:s}.pt'.format(full_decoy_name))
    print(full_decoy_name)
    del data, r_node_list, r_edge_tuple_list, r_edge_feature_list,interface_edge_tuple_list, \
        interface_feature_list,node_list,interface_pos_list,interface_count_list,edge_tuple_list,\
        edge_feature_list,new_edge_tuple_list,node_feature_list,edge_index,node_features, decoy_label, \
...
def read_decoys_and_categories(label_folder, target_names, subset_number):
    ...
    for target in target_names:
        ...
        for item in tqdm(decoy_name_list):
            decoy_name, decoy_label = item.split(" ")
            decoy_names.append(target + "." + decoy_name)
            labels.append(decoy_label)
        del decoy_name_list
    return decoy_names, labels



def process_classification_patch(fir_dir, save_folder , selected_labels, selected_decoy_names):
...
class PPDocking_large_dataset(Dataset):
    ...
    def processed_dir(self):
        return osp.join(self.root, 'processed_all_data', self.mode)
    ...
    def process(self):
        ...
        with open(r'/home/fei/Research/Dataset/zdock_decoy/2_decoys_bm4_zd3.0.2_irad/caseID.lst', 'r') as complex_name_fp:
        ...
        process_classification_patch(self.root, self.processed_dir, all_labels, all_decoy_names)

        del all_complex_names, all_labels,all_decoy_names
        '''
        all_labels = []
        all_decoy_names = ["2C0L.complex.39950.pdb"]
        all_labels.extend(np.zeros(len(all_decoy_names), np.int))

        process_classification_patch(self.root, self.processed_dir, all_labels, all_decoy_names)

        del all_labels,all_decoy_names
