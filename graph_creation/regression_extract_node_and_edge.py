...
def read_decoys_and_categories(label_folder, target_names, subset_number):
    ...
    for target in target_names:
        ...
        non_positive_dataframe = regression_decoy_name_dataframe.loc[regression_decoy_name_dataframe['CAPRI'] == 0].reset_index(drop=True)
        non_positive_list = list(non_positive_dataframe["No"])
        ...
        if non_positive_list!="":
            for item in non_positive_list:
                decoy_name = item
                decoy_label = 0
                if target + "." + decoy_name not in existing_decoy_names:
                    decoy_names.append(target + "." + decoy_name)
                    labels.append(int(decoy_label))
        del non_positive_list, non_positive_dataframe, regression_decoy_name_dataframe
